...
func _update_size() -> void:
    ...
    _visible_items = mini(_candidates.size(), max_lines)
    var available_height := _cache.item_min_size.y * _visible_items
    ...
    if popup_rect.end.y > local_bounding_rect.end.y:
        ...
        _visible_items = clampi(actual_available_height / _cache.item_min_size.y, 0, max_lines)
        available_height = _cache.item_min_size.y * _visible_items
    ...
    _panel_rect = Rect2(Vector2.ZERO, popup_rect.size)

    var content_rect := _panel_rect
    content_rect.position += _cache.panel_start_offset
    content_rect.size -= _cache.panel_total_offset

    # Update scroll bar
    var scroll_bar_rect := Rect2()
    _scroll_bar.visible = _visible_items < _candidates.size()
    if _scroll_bar.visible:
        var scroll_min_size := _scroll_bar.get_combined_minimum_size()
        ...
        _scroll_bar.page = _visible_items

    # Update item rect
    _item_rect = content_rect
    _item_rect.size.y = _cache.item_min_size.y
    if _scroll_bar.visible:
        var scroll_bar_rect_offset := scroll_bar_rect.size.x
        _item_rect.size.x -= scroll_bar_rect_offset
        if is_layout_rtl():
            _item_rect.position.x += scroll_bar_rect_offset

    # Update item content rects
    var item_content_rect := _item_rect
    if is_layout_rtl():
        item_content_rect.position.x += _cache.item_end_padding
    else:
        item_content_rect.position.x += _cache.item_start_padding
    item_content_rect.size.x -= _cache.item_h_padding

    _icon_rect = item_content_rect
    _icon_rect.size.x = ThemeDB.fallback_icon.get_width() * get_theme_default_base_scale()
    if _cache.icon_max_width > 0:
        _icon_rect.size.x = _cache.icon_max_width
    if is_layout_rtl():
        _icon_rect.position.x = item_content_rect.end.x - _icon_rect.size.x

    var icon_rect_offset := _icon_rect.size.x + _cache.h_separation
    _text_rect = item_content_rect
    _text_rect.size.x -= icon_rect_offset
    if not is_layout_rtl():
        _text_rect.position.x += icon_rect_offset
...
func _select(offset: int) -> void:
    ...
    var scroll_margin := _visible_items - 1
...
func _draw() -> void:
    ...
    _cache.panel.draw(ci, _panel_rect)

    var item_rect := _item_rect
    var icon_rect := _icon_rect
    var text_rect := _text_rect
    var rect_offset := _item_rect.size.y
